...
public class NotContainsRuleTransformerTests
{
    ...
    public void Transform_WithMultipleValues_ShouldGenerateAndConditions()
    {
        ...
        // Assert
        Assert.Equal("(\"Description\" NOT LIKE '%' || $20 || '%' AND \"Description\" NOT LIKE '%' || $21 || '%' AND \"Description\" NOT LIKE '%' || $22 || '%')", query);
        Assert.NotNull(parameters);
    ...
    public void Transform_WithList_ShouldGenerateAndConditions()
    {
        ...
        // Assert
        Assert.Equal("(\"Content\" NOT LIKE '%' || $30 || '%' AND \"Content\" NOT LIKE '%' || $31 || '%')", query);
        Assert.NotNull(parameters);
