...
public class ContainsRuleTransformerTests
{
    ...
    {
        // Arrange
        var rule = new FilterRule("Email", "contains", new List<string> { "@spam.com", "@fake.org" });
    ...
    public void Transform_WithNullValue_ShouldThrowArgumentNullException()
    {
        ...
        // Assert
        Assert.Contains("CONTAINS operator requires a non-null value", ex.Message);
    }
    ...
    public void Transform_WithEmptyArray_ShouldThrowArgumentException()
    {
        ...
        // Assert
        Assert.Contains("CONTAINS operator requires at least one value", ex.Message);
    }
}
