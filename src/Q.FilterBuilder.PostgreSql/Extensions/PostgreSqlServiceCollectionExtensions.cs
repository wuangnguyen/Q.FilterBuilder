...
public static class PostgreSqlServiceCollectionExtensions
{
    ...
    public static IServiceCollection AddPostgreSqlFilterBuilder(this IServiceCollection services)
    {
        ...
        return services.AddPostgreSqlFilterBuilder(null);
    }
    ...
    /// </summary>
    /// <param name="services">The service collection.</param>
    /// <param name="configureOptions">Action to configure PostgreSQL FilterBuilder options.</param>
    /// <returns>The service collection for chaining.</returns>
    /// <exception cref="ArgumentNullException">Thrown when services is null.</exception>
    public static IServiceCollection AddPostgreSqlFilterBuilder(
        this IServiceCollection services,
        Action<PostgreSqlFilterBuilderOptions>? configureOptions = null)
    {
        if (services == null)
        {
            throw new ArgumentNullException(nameof(services));
        ...
        return services.AddFilterBuilder(
            ...
            ruleTransformers =>
            {
                ...
                // Add PostgreSQL-specific transformers
                RegisterPostgreSqlTransformers(ruleTransformers);
    ...
    /// <summary>
    /// Registers PostgreSQL specific rule transformers.
    ...
    private static void RegisterPostgreSqlTransformers(IRuleTransformerService service)
    {
...
/// <summary>
/// Configuration options for PostgreSQL FilterBuilder.
/// </summary>
public class PostgreSqlFilterBuilderOptions
{
    ...
    /// <summary>
    /// Configures custom type converters for the PostgreSQL FilterBuilder.
    /// </summary>
    ...
    public PostgreSqlFilterBuilderOptions ConfigureTypeConversions(Action<ITypeConversionService> configure)
    {
        if (configure == null)
        {
    ...
    /// <summary>
    /// Configures custom rule transformers for the PostgreSQL FilterBuilder.
    /// </summary>
    /// <param name="configure">Action to configure rule transformers.</param>
    /// <returns>The options instance for chaining.</returns>
    /// <exception cref="ArgumentNullException">Thrown when configure is null.</exception>
    public PostgreSqlFilterBuilderOptions ConfigureRuleTransformers(Action<IRuleTransformerService> configure)
    {
        if (configure == null)
        {
            throw new ArgumentNullException(nameof(configure));
